...
function generateMathChallenge() {
    ...
    echo '<input type="hidden" name="math_challenge_answer" value="' . $result . '">';
}
?>
<!DOCTYPE html>
<html lang="en-us">
			...
			<div class="row justify-content-center align-items-center">
				<div class="col-lg-6">
					<div class="section-title text-center">
						...
						<p>Fill the form below with your requirements and we will get back to you at the earliest.</p>
					</div>
				</div>
				<div class="col-lg-10">
								...
								<div class="contact-form">
									<form>
                                        <div class="form-group mb-4 pb-2">
                                            ...
                                            <input type="text" class="form-control shadow-none" id="contact_name" name="contact_name">
                                        </div>
                                        <div class="form-group mb-4 pb-2">
                                            <label for="contact_email" class="form-label">Email address</label>
                                            <input type="email" class="form-control shadow-none" id="contact_email" name="contact_email">
                                        </div>
                                        <div class="form-group mb-4 pb-2">
                                            <label for="contact_phone" class="form-label">Mobile Number</label>
                                            <input type="number" class="form-control shadow-none" id="contact_phone" name="contact_phone">
                                        </div>
                                        <div class="form-group mb-4 pb-2">
                                            <label for="exampleFormControlTextarea1" class="form-label">Write Message</label>
                                            <textarea class="form-control shadow-none" id="contact_message" name="contact_message" rows="3"></textarea>
                                        </div>
                                        <label for="math_challenge">Math Challenge: <?php generateMathChallenge(); ?></label>
                                        <input type="text" id="math_challenge" name="math_challenge">
                                        <div style="margin-top: 20px;"></div>
                                        <div id="alert" class="alert alert-danger f-12 text-center" role="alert" style="visibility:hidden; display:none; margin-top: 10px;"> </div>
                                        <button class="btn btn-primary w-100" type="submit" id="signup" onclick="submitform()">Send Message</button>
                                    </form>
								</div>
							</div>
							<div class="col-lg-6 mt-5 mt-lg-0">
								<div class="shadow rounded py-5 px-4 ms-0 ms-lg-4 bg-white position-relative">
									...
									<div class="block mx-0 mx-lg-3 mt-4">
										<h4 class="h5">Chat with us (WhatsApp)</h4>
										<div class="content"><a href="tel:+918595894805">+91 8595894805</a></div>
									</div>
									<div class="block mx-0 mx-lg-3 mt-4">
										<h4 class="h5">Email your concern</h4>
										<div class="content"><a href="mailto:contact@nirvriksh.com">contact@nirvriksh.com</a> </div>
									</div>
								</div>
							</div>
	...
	<script>
        function submitform() {
        	...
        	if(name == '' || email == '' || phone == '' || message == '') {
        	    $('#alert').html('Please fill all the form fields. All fields are required.');
                $('#alert').css("display", "block");
                $('#alert').css("visibility", "visible");
                $('#alert').css("color", "red");
        	}
        	
        	var correctAnswer = $('[name="math_challenge_answer"]').val();
            if (userAnswer !== correctAnswer) {
                $('#alert').html('Incorrect answer to the math challenge. Please try again.');
                $('#alert').css("display", "block");
                $('#alert').css("visibility", "visible");
                $('#alert').css("color", "red");
            }
            ...
            $.ajax({
              url: "actions/contact_form_submission.php",
              type: 'POST',
              cache: false,
              data: senddata,
        ...
        // 			$('#signup').addClass('disabled');
              ...
              error: function(jqXHR, textStatus, errorThrown) {
                ...
                $('#alert').html('Error while submitting the form. Please try again.');
        		$('#alert').css("display", "block");
        		$('#alert').css("visibility", "visible");
        		$('#alert').css("color", "red");
        		$('#signup').addClass('disabled');
